...
- Install **`dms-greeter` from your distro repo** (layered package or image build). Do **not** rely on `dms-greeter install`, `dms-greeter enable`, or `dms-greeter uninstall` - those commands are disabled on immutable systems by CLI policy.
...
If your distro has no `dms-greeter` package, install `greetd` and `quickshell`, then grab the prebuilt binary from the latest release:
...
cd dank-greeter/core
make build
sudo install -Dm755 bin/dms-greeter /usr/local/bin/dms-greeter
```

The UI is embedded in the binary, there is no quickshell config tree to install. Create the cache directory:
