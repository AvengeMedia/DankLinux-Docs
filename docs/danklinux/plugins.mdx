...
> For the in-depth DankMaterialShell plugin developer guide, head to [Plugins → Plugin Development](/docs/dankmaterialshell/plugins-dev). The notes below focus on the broader tooling that DankLinux provides.

DankLinux ships the tooling required to build, test, and distribute plugins for DankMaterialShell. This page focuses on the developer workflow, complementing the end-user [Plugin ecosystem overview](/docs/dankmaterialshell/plugins).
