...
### `listen_addr`
HTTP server address and port. The API has no authentication, so keep it bound to localhost unless you know what you are doing.

**Default:** `127.0.0.1:43654`
...
#### `extract_exif`
Extract EXIF metadata (camera, date taken, GPS, ISO, aperture, focal length) from images under this path.

**Default:** `true`

```toml
extract_exif = false  # Skip EXIF parsing for code repos
```

#### `exclude_dirs`
List of directory names to skip during indexing. Supports both exact names and regex patterns.

Regex patterns are wrapped in `/` delimiters (e.g., `/^build-/`). Each pattern is matched against individual directory name components, not the full path. Invalid regex patterns are skipped with a warning.

By default, `exclude_dirs` **replaces** the built-in defaults — anything you list is the complete set of excluded directories for that path. Set [`merge_default_exclude_dirs`](#merge_default_exclude_dirs) to `true` if you want your list merged onto the defaults instead.
...
#### `merge_default_exclude_dirs`
When `true`, the path's `exclude_dirs` list is **merged** with the built-in defaults (deduplicated) instead of replacing them. Handy when you only want to add a few extras (like a `secrets/` directory) without copy-pasting the full defaults array.

**Default:** `false` (existing behavior — `exclude_dirs` replaces the defaults)
...
This is especially useful for indexing dotfiles repos: you want `exclude_hidden = false` so directories like `.config` are visible, but you still want noise like `.git/` and `node_modules/` excluded.
...
- Run `dsearch index check <path>` to see why a path is excluded
