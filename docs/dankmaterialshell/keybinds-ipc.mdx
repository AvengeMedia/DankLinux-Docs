...
| `cycle` | Cycle activities (`home` → `media` when available → `launcher` when Default Opens is Island) | - |
...
- `dash` `open` / `toggle` with `media`, `wallpaper`, or `weather`, and `dash close` while those activities are open
