...
**Dank Island:** One bar instance may be designated as Dank Island (Settings → Island → Island instance). It is still listed by `bar` selectors (`index` includes it), but visibility and auto-hide commands return `BAR_IS_ISLAND` — use the [`island`](#island) target instead. `setPosition` accepts only `top` and `bottom` for that instance.

### island

Dank Island activity surface. Requires a bar instance designated as the island in Settings and enabled. Commands target the focused screen unless you use an `*On` variant.
...
`spotlight` and `launcher` open the island launcher when **Launcher → Default Opens** is Island, and fall back to Spotlight if the focused screen has no island.
...
When Dank Island is the sole top chrome on the focused display, `control-center` and `notifications` open island activities instead of their popouts. The notification modal still opens if the island does not own the screen.
...
On displays where Dank Island is the sole top chrome, `open`/`toggle` with `media`, `wallpaper`, or `weather` route to the matching island activity. `close` collapses those activities when they are open.
