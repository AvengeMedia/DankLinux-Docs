...
- **Color Profile** — browse to a `.icc`/`.icm` file and apply it. The row shows the profile description together with version, color space and whether the ramp is currently active; the remove button clears the assignment. The info button next to it opens the profile details: version, class, color space, tone curve, video card gamma table, white point with its color temperature, file size, modification time and path.
...
- ICC ramps are applied even while the night light schedule is disabled: outputs with a profile get their measured ramp, outputs without one stay at a neutral ramp so nothing is tinted unexpectedly.
...
- Only a profile that carries a `vcgt` table can be applied: that tag holds the video card gamma table the GPU ramp takes. The `rTRC`/`gTRC`/`bTRC` tags describe the display's own transfer function (roughly `t^2.2` for a normal monitor), so `dms icc info` reports them but they are never written into the ramp — applying a profile without `vcgt` is refused instead of washing the display out.
...
- DMS only ever writes `iccProfiles` and `outputTemps` to that file: the night light fields are owned by the shell, so applying a profile never persists the night light state the daemon happened to hold.
