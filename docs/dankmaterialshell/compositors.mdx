...
// Open DMS windows as floating by default (optional - DMS can manage this for you, see tip below)
window-rule {
...
:::tip[DMS-managed floating rule]
DMS can manage this rule itself via **Settings → Theme & Colors → Floating Windows → Open Windows Floating** (on by default). The rule lives in a managed fragment at `~/.config/niri/dms/windowrules.kdl`; if that fragment isn't included in your config yet, a **First Time Setup** banner on the same page creates it and adds the include when you click **Setup**. DMS floating windows use app ID `com.danklinux.dms` — remove any hand-written rules targeting the old `org.quickshell` ID.
:::
...
Hyprland must be started with the Lua config for any of this to apply:

```sh
Hyprland -c ~/.config/hypr/hyprland.lua
```

If Hyprland warns that it is using an autogenerated config, or the warning names `hyprland.conf`, the session is not on the Lua config yet.

`dms setup` writes the main config plus fragments under `~/.config/hypr/dms/`:

| Fragment | Purpose | Settings page |
| --- | --- | --- |
| `colors.lua` | Theme colors | Theme & Colors |
| `layout.lua` | Layout, gaps, borders, decoration | Compositor |
| `outputs.lua` | Monitors and display settings | Displays |
| `cursor.lua` | Cursor settings | Cursor |
| `binds.lua` | DMS-managed default shortcuts | - |
| `binds-user.lua` | Your shortcut overrides | Keyboard Shortcuts |
| `windowrules.lua` | Window rules | Window Rules |

`binds.lua` is refreshed by `dms setup`, so put custom shortcuts in `binds-user.lua` or use the Keyboard Shortcuts page. Deleting a DMS default there saves an `hl.unbind("KEY")` line into `binds-user.lua` so the removal sticks across setup runs; **Reset to default** drops your override. Binds in your own `hyprland.lua` are read-only in Settings, DMS does not write files it does not manage.

Stock configs bind a 3-finger horizontal touchpad gesture to workspace switching (`hl.gesture` in `binds.lua`). Add your own `hl.gesture(...)` lines to `binds-user.lua` to change it, or drop a stock gesture with `action = "unset"` matching its fingers, direction, and modifiers.

#### Migrating from hyprland.conf

`dms setup` moves legacy `hyprland.conf` and `dms/*.conf` into `~/.config/hypr/.dms-backups/<timestamp>/` so Hyprland never sees both formats at once. It also migrates `monitor = ...` lines into `dms/outputs.lua` when that file is empty or missing; an existing `outputs.lua` is left alone.

Hyprland still autogenerates `hyprland.conf` when launched without `-c`. DMS sweeps stray copies into the backup folder on the next `dms run`, so if one keeps reappearing the session is not starting from `hyprland.lua`.
...
-- Open DMS windows as floating by default (optional — DMS can manage this for you)
hl.window_rule({ match = { class = "^(com.danklinux.dms)$" }, floating = true })
```

As on niri, **Settings → Theme & Colors → Floating Windows → Open Windows Floating** can manage this rule in `~/.config/hypr/dms/windowrules.lua` (click **Setup** on the banner if the fragment isn't included yet). DMS floating windows use app ID `com.danklinux.dms`.
