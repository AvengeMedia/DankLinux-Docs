...
DankMaterialShell works with any Wayland compositor, but we provide optimized configurations for niri and Hyprland. These configs include DMS-specific keybindings, proper layer shell rules, and recommended settings.

## niri Configuration

niri uses the KDL configuration format. DankMaterialShell integrates seamlessly with niri's scrollable tiling layout.

**Note:** niri's configuration is *not* repeatable, the below settings are snippets, you need to merge them with any existing configuration. Example, you cannot have multiple `layout {}` sections.

### Quick Setup

When you run `dms` for the first time, it will offer to install a recommended niri configuration. Alternatively, you can manually configure niri using the examples below.

### Key Configuration Sections

In the next niri release, you can synchronize gaps and colors with dms automatically, but please note it is **not supported** on the current stable niri release.
