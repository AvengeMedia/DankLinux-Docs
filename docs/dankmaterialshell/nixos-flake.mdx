...
DankMaterialShell is available in nixpkgs stable (26.05). If you're on NixOS 26.05+, the [Installation - NixOS](nixos) method is recommended and doesn't require flakes.
:::
...
If you want to import dgop from the flake, first add it to your flake inputs:
...
:::warning
Note that `niri-stable` package from niri-flake is on version `25.08`, while the needed version for DankMaterialShell is the latest `26.05` (available in nixpkgs).

`niri-flake`, while supported by DankMaterialShell, may not be ideal if you want to configure newer niri features like Background Effects or the Recent Windows switcher. A popular alternative is [niri-nix](https://codeberg.org/BANanaD3V/niri-nix), but you can just use home-manager or hjem to write your niri config directly. In both cases, the DMS integration files live under `~/.config/niri/dms`.
:::
...
You can also export the current in-memory state from the command line, which is easier to script:

```bash
dms ipc call settings dump        # settings.json content
dms ipc call settings dumpSession # session.json content
```

Note which file a key belongs to: runtime state like night mode (`nightModeEnabled`, `nightModeAutoEnabled`, the schedule), the current wallpaper, and light/dark mode are **session** keys - putting them under `settings` does nothing. Also keep in mind that managing `session` declaratively makes runtime session state read-only: toggles like night mode or wallpaper changes from the control center will not persist across generations.
