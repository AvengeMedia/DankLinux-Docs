...
The Quickshell UI is embedded into the `dms` binary at build time. The shell files installed to `/usr/local/share/quickshell/dms` are used by the [greeter](../dankgreeter); the shell itself runs from the embedded UI unless `-c` or `DMS_SHELL_DIR` points elsewhere.
