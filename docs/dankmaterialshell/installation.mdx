...
Shared widgets live in the [dank-qml-common](https://github.com/AvengeMedia/dank-qml-common) submodule, so clone with `--recurse-submodules`:

```bash
git clone --recurse-submodules https://github.com/AvengeMedia/DankMaterialShell.git ~/dms
```

In an existing clone, run `git submodule update --init` instead — `make install` fails with a `DankCommon missing` error until the submodule is present.
...
The Quickshell UI is embedded into the `dms` binary at build time. The shell runs from the embedded UI unless `-c` or `DMS_SHELL_DIR` points elsewhere. The login screen is a separate binary from [dank-greeter](https://github.com/AvengeMedia/dank-greeter) with its own embedded UI.
