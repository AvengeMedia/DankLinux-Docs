...
# Install udev rules for brightness/backlight/led control
sudo make install-config
```

#### 3.1 Add user to groups

`video` is usually required to control display/backlight devices, while `input` is usually required to control LEDs (such as keyboard lighting)
