...
The recommended option for Gentoo is the official [guru](https://wiki.gentoo.org/wiki/Project:GURU) community overlay, which packages DMS (`gui-apps/dankmaterialshell`) alongside companion tools `gui-apps/dgop` and `gui-apps/danksearch`.

### guru (Recommended)

Enable the guru overlay and install DMS:
...
Install the optional companion tools:
...
The [quilat-overlay](https://github.com/Graght/quilat-overlay) provides a live ebuild (`gui-shell/dms-9999`) with OpenRC and systemd support. This tracks git master, so updates come via `emerge --sync && emerge -uDN @world`.
...
The [tdgentoo](https://github.com/timdodge/tdgentoo) overlay provides a versioned ebuild (`gui-wm/DankMaterialShell`) pinned to stable releases.
...
The [dacyberduck overlay](https://codeberg.org/dacyberduck/gentoo-overlay) provides DMS under `dank-base/dankmaterialshell`.
...
Void packages are pending submission to the official [void-packages](https://github.com/void-linux/void-packages) repository. Once merged, install with:
...
Until then, build from source with `xbps-src`. Every runtime dependency except `dgop` and `danksearch` is already in the Void repositories; the `dms`, `dms-greeter`, `dgop`, and `danksearch` templates are maintained in the [DankMaterialShell](https://github.com/AvengeMedia/DankMaterialShell/tree/master/distro/void) and [danklinux](https://github.com/AvengeMedia/danklinux/tree/master/distro/void) repos.
...
# Copy in the DMS templates, then build (dms requires dgop)
./xbps-src pkg dgop
...
Only `quickshell` is strictly required. Install optional features as desired — `matugen` (theming), `cava` (audio visualiser), `danksearch` (search), `qt6-multimedia` (sounds) — and a compositor such as `niri`, `hyprland`, or `sway`. See the [Void README](https://github.com/AvengeMedia/DankMaterialShell/tree/master/distro/void) for the full optional-dependency list and greeter setup.
