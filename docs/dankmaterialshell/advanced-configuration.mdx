...
Frame mode is separate from fullscreen policy. When a visible Frame is enabled,
bar and dock surfaces may internally use `overlay` so their icons stack above
the frame in both Separate and Connected modes. Fullscreen windows are still
handled by the compositor and by the per-surface layer choices above.
