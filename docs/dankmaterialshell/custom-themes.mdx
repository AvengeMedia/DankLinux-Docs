...
DankMaterialShell supports custom color themes using Material Design 3 color schemes. You can create your own themes or use pre-made ones.

## Theme Structure

Custom themes are defined in JSON files with complete Material Design 3 color specifications. Themes can have separate `dark` and `light` variants, or use a single color scheme for both modes.
