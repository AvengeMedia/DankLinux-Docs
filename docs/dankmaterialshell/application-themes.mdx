...
Your templates have access to all standard [matugen color keywords](https://iniox.github.io/#matugen/templates/material-design-3), plus DMS provides the full `dank16` palette for terminal and editor theming.
...
Each variant includes all standard [matugen color formats](https://iniox.github.io/#matugen/templates/color-formats).
...
With adw-gtk3 installed, DMS copies the theme (light and dark variants) into `~/.local/share/themes` and patches the copies with your generated colors. GTK3 applications then follow the system color-scheme, switching between light and dark live without a restart. When the system theme updates, the copy refreshes on the next theme apply; a copy you created yourself is never touched.

Without adw-gtk3, GTK3 falls back to a global `dank-colors.css` import in `~/.config/gtk-3.0/gtk.css`, and GTK4 uses the same import mechanism in `~/.config/gtk-4.0`.

`dms doctor` reports which mode is active under **Optional Features → adw-gtk3**.
...
### VSCode / Codium / Cursor / Windsurf / Positron
...
# or: codium / cursor / windsurf / positron --install-extension ...
```
