...
Zen Browser handles theming differently from Firefox—Pywalfox and Firefox theme extensions no longer work. Theming is controlled via `userChrome.css` in the profile directory. Zen requires a browser restart for theme changes to take effect—live reloading is not supported.
