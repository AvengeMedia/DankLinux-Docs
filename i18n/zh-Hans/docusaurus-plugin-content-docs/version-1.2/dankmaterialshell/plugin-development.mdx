...
确保仅键入您想要的命名空间，并且不要添加前缀（例如 `dms:` 或 `dms-plugin:`），因为 shell 会自动添加 `dms-plugin:` 作为前缀。
例如，下面插件的命名空间将为 `dms-plugin:emoji-launcher`。

虽然您不必向小部件添加图层命名空间（它将回退到 `dms-plugin:plugin`），但最好这样做。
:::warning
...
启动器插件向 Spotlight 搜索添加项目。它们使用 `QtObject` 作为根（不是 `Item` 或 `PluginComponent`），因为它们不直接渲染任何 UI。
...
|属性/函数|描述 |
|-------------------|-------------|
| `pluginService` | 用于数据持久化的自动注入服务 |
| `trigger` | 激活插件的前缀（例如，`"="` 或 `"#"`）|
| `itemsChanged` | 当您的项目列表更新时发出的信号 |
| `getItems(query)` | 返回与查询匹配的筛选项目 |
| `executeItem(item)` | 处理项目选择 |
| `getContextMenuActions(item)` | *（可选）*返回项目的上下文菜单操作 |

### 上下文菜单操作 {#context-menu-actions}

通过实施 `getContextMenuActions(item)` 为启动器项目添加右键单击上下文菜单：
...
启动器清单需要 `"type": "launcher"` 和 `"trigger"`：
...
- `icon` - 要显示的图标（见下方格式）
- `comment` - 辅助文本/描述
- `action` - 选择时发生的事情（见下方格式）
- `categories` - 类别字符串数组
- `keywords` - 用于搜索匹配的字符串数组（例如，`["math", "calculate"]`）

**图标格式**：
- Material Design：`"material:icon_name"` 或仅 `"icon_name"`
- Unicode/Emoji：`"unicode:🚀"`

**动作格式**：
- 复制到剪贴板：`"copy:text"`
- 显示 toast：`"toast:message"`
- 运行脚本：`"script:command args"`
...
检查 DMS 存储库中的 `PLUGINS/` 目录以获取真实示例：
