...
DankMaterialShell 可与任何 Wayland 合成器配合使用，但我们为 niri、Hyprland、Sway、MangoWC、labwc 和 Miracle WM 提供优化配置。这些配置包括 DMS 特定的键绑定、适当的层 shell 规则和推荐的设置。
...
对于systemd自启动设置，请参阅[安装指南](./installation#niri)。如果您不想使用 systemd，请将 DMS 直接添加到您的 niri 配置中：
...
Hyprland 在 `~/.config/hypr/hyprland.lua` 使用基于 lua 的配置。
...
对于 systemd 自动启动设置（如果您有多个桌面环境，建议使用），请参阅[安装指南](./installation#hyprland)。如果您不想使用 systemd，请将 DMS 直接添加到您的 Hyprland 配置中：
...
对于 systemd 自动启动设置（如果您有多个桌面环境，建议使用），请参阅[安装指南](./installation#mangowc)。如果您不想使用 systemd，请将 DMS 直接添加到您的 MangoWC 配置中：
...
对于 systemd 自动启动设置（如果您有多个桌面环境，建议使用），请参阅[安装指南](./installation#sway)。否则，请将 DMS 直接添加到您的 Sway 配置中：
...
## 奇迹 WM 配置 {#miracle-wm-configuration}

[Miracle WM](https://github.com/miracle-wm-org/miracle-wm) 是一个基于 Mir 构建的平铺 Wayland 合成器。

### 关键配置部分 {#key-configuration-sections-5}

:::info[即将推出]

详细的 Miracle WM 配置部分即将推出。 Miracle WM 配置将包括：
- 启动命令
- 环境变量
- DMS 键绑定
- 窗口规则
...
对于 systemd 自动启动设置（如果您有多个桌面环境，建议使用），请参阅[安装指南](./installation#miracle-wm)。否则，将 DMS 直接添加到您的 Miracle WM 配置中。

#### DMS 键绑定 {#dms-keybindings-5}

有关可用 IPC 命令的完整列表，请参阅 [IPC](./keybinds-ipc.mdx)。 Miracle WM 键绑定配置即将推出。
...
3. **在 `~/.config/environment.d/90-dms.conf`（对于 systemd 用户）或您的合成器配置中设置环境变量**：
   ...
   QT_QPA_PLATFORM=wayland
   ```
您还可以使用 `systemctl --user edit dms` 并在 `[Service]` 下添加 `Environment=QT_QPA_PLATFORM=wayland`。这仅影响 DMS 及其启动的应用程序，而 `90-dms.conf` 适用于所有用户会话。
...
检查合成器的日志以确保 `spawn-at-startup` 或等效启动命令正在运行。您还可以使用 `dms run` 手动启动 DMS 以查看任何错误消息。
