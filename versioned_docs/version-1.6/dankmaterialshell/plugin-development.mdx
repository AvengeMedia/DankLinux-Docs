...
Provide any subset of the four surface keys — you don't need all of them. A daemon + bar widget is just as valid as all four.
...
`components` replaces `component` for [composite plugins](#composite-plugins) that expose more than one surface. Provide one or the other.
