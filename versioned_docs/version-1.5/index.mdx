...
The installer adds the appropriate repository (COPR, OBS, or PPA) or installs from official repos and installs packages through your distro's package manager. Future updates come through normal system updates. If you install DMS directly from packages, run `dms setup` to generate starter compositor and terminal configs (niri/Hyprland only). Other compositors like sway, MangoWC, labwc, and Miracle WM are supported—see [manual configuration](dankmaterialshell/installation#post-install).
...
- **[dcal](dankcalendar/)** - A standalone calendar app that unifies Local, Google, Microsoft, CalDAV, and iCloud calendars.
...
### dankcalendar - All Your Calendars in One Place

Calendars on Linux usually mean wrestling with `vdirsyncer` and `khal` config files just to see your Google or iCloud events. DankCalendar replaces that with a single standalone app.

`dcal` brings Local, Google, Microsoft, CalDAV, and iCloud calendars together in one unified agenda. It runs as a lightweight daemon with a tray icon, keeps everything synced in the background, and fires native reminders for upcoming events — all with the look and feel of DankMaterialShell.

- **Unified Agenda**: Every account and calendar in one view.
- **Background Sync**: A lightweight daemon keeps accounts synced and serves the UI over IPC.
- **Secure**: OAuth tokens and secrets live in your system keyring, not in plaintext config files.
- **Scriptable**: A full IPC surface for keybinds and automation.
...
Ready to dive in? Head over to the [Getting Started](./getting-started.mdx) guide to begin your Dank Linux journey!
