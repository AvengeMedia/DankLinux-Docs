...
**Manage greetd PAM automatically** in **Settings → Greeter**. With the
toggle off, DMS never writes to `/etc/pam.d/greetd`: the next sync removes
...
The lock screen resolves the system PAM auth stack automatically by default.
If your setup needs a specific PAM service (LDAP, Kerberos, `systemd-homed`,
or any custom stack), pin one in **Settings → Lock Screen →
Authentication Source**: choose a detected service such as `system-auth`, or
select **Custom…** and enter an absolute path to a PAM service file. Every
selection is validated before it is applied, and an invalid file is never
applied. If the pinned file later disappears, the lock screen automatically
falls back to **Auto**.

When the pinned stack already includes `pam_fprintd` or `pam_u2f`, DMS
suppresses its own separate fingerprint and security-key prompts so you are
not prompted twice.
